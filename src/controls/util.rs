...
use smash::app::{self, sv_battle_object};
use smash::lib::{*, lua_const::*};
use smash::lib::L2CValue;
use crate::controls::FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID;
...
/// Get a fighter's BattleObjectModuleAccessor by fighter index (0-7).
pub unsafe fn get_boma(fighter_index: usize) -> Option<&'static mut BattleObjectModuleAccessor> {
    let entry_id = fighter_index as i32;
    let boma = app::sv_battle_object::module_accessor(entry_id as u32);
    if boma.is_null() {
        return None;
    }
    Some(&mut *(boma as *mut BattleObjectModuleAccessor))
}


pub unsafe fn get_player_number(boma: *mut smash::app::BattleObjectModuleAccessor) -> usize {
    unsafe {
        smash::app::lua_bind::WorkModule::get_int(boma, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize
    }
}
