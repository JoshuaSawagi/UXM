...
unsafe extern "C" fn laser_cancel(fighter : &mut L2CFighterCommon) {
    unsafe {
        let boma = smash::app::sv_system::battle_object_module_accessor(fighter.lua_state_agent); 
		if is_default(boma) {
				...
				if [*FIGHTER_STATUS_KIND_SPECIAL_N].contains(&status_kind) {
					...
					if StatusModule::situation_kind(boma) == *SITUATION_KIND_AIR {
						...
						if (cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_FALL_JUMP) != 0 && stick_y < -0.66 && SPEED_Y[ENTRY_ID] <= 0.0 {
							...
							// Check for shield button or another specific button to trigger the cancel
...
pub fn install() {
    Agent::new("wolf")
	.on_line(Init, fang_cancel)
	.on_line(Main, laser_cancel)
    .install();
}